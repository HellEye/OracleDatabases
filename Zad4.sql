...
BEGIN
    ...
    CREATE TABLE myszy (
        ...
        waga_myszy     NUMBER NOT NULL CONSTRAINT my_wm_ck CHECK (waga_myszy BETWEEN '||waga_min||' AND '||waga_max||'),
...
BEGIN
    ...
    WHILE kiedy <= '2016-01-18' LOOP
        ...
        FROM Kocury WHERE w_stadku_od < sroda;
        
        FOR i IN 1..ile LOOP
            INSERT INTO Myszy(lowca, zjadacz, waga_myszy, data_zlowania, data_wydania)
            VALUES (
                       pseuda(DMBS_RANDOM.VALUE(1, psueda.COUNT)),
                       NULL,
        ...
        FROM kocury ORDER BY zjada DESC
            ...
            WHERE zjadacz IS NULL AND ROWNUM <= kot.zjada;
